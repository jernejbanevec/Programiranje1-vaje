...
let rec make_move player field =
	(match player with
	| Field.Circle -> print_string "Circle";
	| Field.Cross -> print_string "Cross";
	| Field.Empty -> print_string "Empty";);
  ...
  try
    ...
    (*/// Nastavi simbol na koordinatah x in y v polju field na znak igralca
          player. ///*)
  Field.set_symbol x y player fiald
  with
  ...
  | Field.Coordinate_out_of_bounds ->
    print_endline "Stopil si izven igralnega polja!"; make_move player field
  | Field.Nonempty_value_at_coordinates ->
    print_endline "Koordinate ne smejo biti prazne!"; make_move player field
  | Failure msg when msg = "int_of_string" ->
    print_endline "Intigerje sem pretvoril v string"; make_move player field
